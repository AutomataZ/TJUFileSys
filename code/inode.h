...
public:
    ...
    /// @param blkno 关联的块号
    void appendBlk(std::fstream&disk, SuperBlock& s, int inode_index, int blkno);
    
    /// @brief 释放inode之前，释放该inode关联的所有盘块
    /// @param disk 需要写入的磁盘文件
    /// @param s 内存superblkock
    void releaseAllBlk(std::fstream&disk, SuperBlock& s);
    ...
    int BMap(std::fstream& disk, int blkno);
